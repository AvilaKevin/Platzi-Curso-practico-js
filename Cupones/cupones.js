var misCupones = [
  // Se creo un array con varios objetos.
...
function calculaDescuento(valor, descuento) {
  // funcion para realizar el calculo del valor a pagar con descuento.
...
function resultadoCupones() {
  ...
  function Operacion() {
    ...
    var busca = misCupones.find(
      // esto realiza un filtro dentro del array, el cual me trae el valor especificado si es que lo hay.
      function (cuponex) {
        return cuponex.cupon === inputcupones; // el inputcupones es el valor que necesito o el q solicite validar
      }
    );

    // Se crea un condicional para mostrar al usuario si el cupon es valido o no.
    if (busca === undefined) {
      // el undefined es el valor que trae la var busca cuando el cupon es invalido.
